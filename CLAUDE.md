...
```json
{
  ...
  "tag_color": "yellow"  // optional: "yellow" → promoted (gold), "purple" → promoted_violet, "red" → promoted_red
}
```
...
**Tag-color sniffing**: if `tag_color` is omitted (or set to an unrecognized value), the API examines the description text and picks a color from its leading word — case-insensitive match against `yellow` / `purple` / `red`. This makes the convention of putting the color name as the first word of the YouTube description work automatically. To disable or override, pass an explicit `tag_color` in the body. The mapping `color → bucket` is centralized in `ApiVideos::TAG_COLOR_BUCKETS` — add a new color there + a sibling array in `json/promoted_tags.json` + a `.video-tag-promoted-<color>` CSS rule to extend.
