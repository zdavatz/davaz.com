...
A type-ahead **search bar** above the grids filters all three sections by title and description in real time. The bar has a prominent violet border so it stands out against the page, and its placeholder rotates through sample queries (e.g. `chick`, `Georgien`, `Kazakh`) to hint at searchable content. Above the bar, a **tag cloud** of the top ~40 keywords (extracted from video titles only, stopwords filtered) is rendered server-side; clicking a tag fills the search field and filters the grids. Curated tags live in `json/promoted_tags.json` and are loaded with mtime-based reload (edit the JSON and the change picks up on the next request — no service restart needed). The file has three arrays: `promoted` (gold styling, draws attention), `promoted_violet` (violet styling, same look as derived tags but manually curated — used for multi-word labels like `Male Mating` or description-only matches like `Prix de Bâle`), and `stopwords` (tokens excluded from the derived tag list, used both for common words like `the`/`und`/`il` and for deduplicating gold-label tokens like `castelbel` so they don't also render as derived tags). Each entry is `[display label, search query]`. Promoted tags whose query matches no video at all are silently filtered out (`active_promoted_tags` / `active_promoted_violet_tags`), so every visible tag is guaranteed to surface at least one result. The remaining gold tags are spread evenly across the violet tags rather than grouped together. Clearing the search restores the original grid layout with scroll-to-load.
