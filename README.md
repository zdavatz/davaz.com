...
`tag_color` is optional: `"yellow"` appends to `promoted` (gold), `"purple"` to `promoted_violet`, `"red"` to `promoted_red`, `"green"` to `promoted_green`. If omitted, the server sniffs the YouTube description for one of those color names as a whole word (earliest occurrence wins) — so simply writing the color name in the description picks the bucket automatically. Status codes: `401` (bad token), `409` (URL already exists, returns existing `id`), `422` (missing/unrecognized URL), `405` (non-POST), `502` (YouTube API failed).
...
A type-ahead **search bar** above the grids filters all three sections by title and description in real time. The bar has a prominent violet border so it stands out against the page, and its placeholder rotates through sample queries (e.g. `chick`, `Georgien`, `Kazakh`) to hint at searchable content. **Whitespace-insensitive matching**: after a raw substring miss, the search compares query and field with all whitespace stripped, so `historie` finds `H I S T O R I E` and `wife?` finds `W   I   F   E  ?` regardless of spacing variants. Above the bar, a **tag cloud** of the top ~40 keywords (extracted from video titles only, stopwords filtered) is rendered server-side; clicking a tag fills the search field and filters the grids. The cloud spans the full viewport width (matching the ticker) and is **randomly shuffled on every page load**, so curated tags disperse naturally throughout the cloud instead of clustering. Curated tags live in `json/promoted_tags.json` and are loaded with mtime-based reload (edit the JSON and the change picks up on the next request — no service restart needed). The file has four color arrays plus stopwords: `promoted` (gold, draws attention), `promoted_violet` (violet, same look as derived tags but manually curated — used for multi-word labels like `Male Mating` or description-only matches like `Prix de Bâle`), `promoted_red` (pale-red pill, dark-red text), `promoted_green` (pale-green pill, dark-green text), and `stopwords` (tokens excluded from the derived tag list, used both for common words like `the`/`und`/`il` and for deduplicating gold-label tokens like `castelbel` so they don't also render as derived tags). Each entry is `[display label, search query]`. Promoted tags whose query matches no video at all are silently filtered out (same whitespace-insensitive rule as the client-side search, so a curated query like `w i f e ?` activates for titles with different spacing). Clearing the search restores the original grid layout with scroll-to-load.
