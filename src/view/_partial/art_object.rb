  ...
  class ArtObjectInnerComposite < HtmlGrid::DivComposite
    ...
    def youtube_embed(model=@model)
      ...
      if DaVaz::Util::YoutubeHelper.clip_url?(model.url)
        ...
        %(<a href="#{model.url}" target="_blank" class="movies-embed-wrapper" style="display:block;text-decoration:none"><img src="https://img.youtube.com/vi/#{video_id}/maxresdefault.jpg" alt="#{video_id}" class="movies-embed-thumbnail"><div class="movies-embed-play"></div></a>#{views_html})
      else
