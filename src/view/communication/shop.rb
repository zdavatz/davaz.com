#!/usr/bin/env ruby
# View::Communication::Shop -- davaz.com -- 19.09.2005 -- mhuggler@ywesee.com

require 'htmlgrid/divform'
require 'view/formlist'
...
class ShoppingCartList < View::List
	...
	def remove_item(model)
		...
		url = @lookandfeel.event_url('communication', 
			'ajax_shop', args)
		input_field_id = "article[#{model.artobject_id}]"
		script = "removeFromShoppingCart('#{url}', '#{input_field_id}');"
...
class Shop < View::CommunicationPublicTemplate
	CONTENT = View::Communication::ShopComposite
end
		end
	end
end
