...
class PwServer
  def self.show_test_password_yaml
...
class TestPwServer < Minitest::Test
  ...
  def test_generate_token
    result = @pw_server.login("good_user1", "good_password1")
    assert(result.is_a?(PwServer::PwEntry), "PwEntry must succed for good_user1 but was #{result}")
    token = result.generate_token
    expected = "3276163622155287083"
    assert_equal(expected, token, "generate_token should be #{expected} but was #{result}")
  end
  ...
  def test_generate_name
    ...
    name = result.name
    expected = "good_user1"
    assert_equal(expected, name, "generate_name should be #{expected} but was #{result}")
  end
end
